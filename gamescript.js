...
var Snake = (function () {
  ...
  var game = {
    ...
    RandomCandy: function () {
      if(walls){
        candy.x = 1+Math.floor(Math.random() * (tileCount-2));
        candy.y = 1+Math.floor(Math.random() * (tileCount-2));
      }
      else {
    ...
    loop: function () {
      ...
      if (player.x == candy.x && player.y == candy.y) {
        ...
        reward = 1;
        game.RandomCandy();
        // make sure new candy didn't spawn in snake tail
        while((function () {
          for(var i=0; i<trail.length; i++) {
            if (trail[i].x == candy.x && trail[i].y == candy.y) {
              game.RandomCandy();
              return true;
            }
          }
      ...
      ctx.fillStyle = 'red';
      ctx.fillRect(candy.x * gridSize+1, candy.y * gridSize+1, gridSize-2, gridSize-2);
